...
int main( int argc, char *argv[])
{
   ...
   *-------------------------------------------------*/
   ...
   init_pre_process(&preproc_s);
   init_coder_ld8a(&cod_s);           /* Initialize the coder             */
   ...
   while( fread((void *)sp16, sizeof(INT16), L_FRAME, f_speech) == L_FRAME){
      ...
      for (i = 0; i < L_FRAME; i++)  cod_s.new_speech[i] = (FLOAT) sp16[i];

      pre_process(&preproc_s, cod_s.new_speech, L_FRAME);

      coder_ld8a(&cod_s, prm);
