...
void lsp_encw_reset(struct lsp_cod_state_t *);
...
void lsp_get_quant(
  const FLOAT lspcb1[][M],
  const FLOAT lspcb2[][M],
  int code0,
  int code1,
  int code2,
  const FLOAT fg[][M],
...
void lsp_prev_extract(
  FLOAT lsp[M],
  FLOAT lsp_ele[M],
  const FLOAT fg[MA_NP][M],
  FLOAT freq_prev[MA_NP][M],
  const FLOAT fg_sum_inv[M]
);
...
int qua_gain(struct gain_state_t *,
		FLOAT code[], FLOAT *coeff, int lcode, FLOAT *gain_pit,
        FLOAT *gain_code, int tameflag   );

void  dec_gain(struct gain_state_t *,
			   int indice, FLOAT code[], int lcode, int bfi, FLOAT *gain_pit,
               FLOAT *gain_code);
