...
static void lsp_prev_compose(
  ...
  const FLOAT fg[][M],               /* (i) Q15 : MA prediction coef.         */
  FLOAT freq_prev[][M],        /* (i) Q13 : previous LSP vector         */
  const FLOAT fg_sum[]               /* (i) Q15 : present MA prediction coef. */
)
{
   int j, k;
...
void lsp_prev_extract(
  ...
  const FLOAT fg[MA_NP][M],          /* (i) Q15 : MA prediction coef.         */
  FLOAT freq_prev[MA_NP][M],   /* (i) Q13 : previous LSP vector         */
  const FLOAT fg_sum_inv[M]          /* (i) Q12 : inverse previous LSP vector */
)
{
  int j, k;
