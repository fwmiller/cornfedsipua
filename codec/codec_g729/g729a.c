...
void
g729_init_coder(struct cod_state * cod)
{
...
g729_coder(struct cod_state * cod, INT16 *DataBuff, char *Vout, int* poutlen)
{
	...
	coder_ld8a(&cod->cod_s, parm + 1);
	parm[0] = G729_TYPE_VOICE;

	if (parm[0] != G729_TYPE_DONTSEND)
		g729_line_pack(parm, (unsigned char *) Vout, poutlen);
	else
		*poutlen = 0;
...
g729_decoder(struct dec_state * dec, INT16 *DataBuff, char *Vinp, int inplen)
{
	...
	g729_line_unpack(parm, (unsigned char *) Vinp, Ftyp);

	decod_ld8a(&dec->dec_s, parm, dec->synth, Az_dec, T2);
	post_filter(&dec->postfilt_s, dec->synth, Az_dec, T2);
